class Game { //<>//
  ...
  //Entity fields
  public Human player;
  public final GameObjectHandler objectHandler = new GameObjectHandler();
  private IVector exitDoorPosition;
  ...
  private float mapDisplayWidth;
  private float mapDisplayHeight;
  ...
  private ArrayList<StatOverlay> statOverlays = new ArrayList<StatOverlay>();
  ...
  public void onTurn() {
    ...
    //Room exit
    if (player.getPosition().equals(exitDoorPosition)) {
  ...
  private void initNewMap() {
    ...
    tileMap.setTile(new BaseTile(TileType.REGULAR, false), exitDoorPosition);
  }
  ...
  private void initNewLevel(boolean initNewPlayer, boolean killMobs) {
    ...
    exitDoorPosition = null;
    initNewMap();
    if (exitDoorPosition == null) {
      exitDoorPosition = new IVector(settings.mapWidth - 2, settings.mapHeight - 2);
    }
  ...
  public void setExitDoorPosition(IVector pos) {
    exitDoorPosition = pos.copy();
  }
  ...
  private void drawExitDoor() {
    fill(#984310);
    noStroke();
    rect(exitDoorPosition.x * tileWidth + mapOffsetX, exitDoorPosition.y * tileHeight + mapOffsetY, tileWidth, tileHeight);
  }
