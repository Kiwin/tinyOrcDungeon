class Game {
  ...
  public void onTurn() {
    ...
    if (player.getPosition().isEqualTo(new IVector(settings.mapWidth - 2, settings.mapHeight - 2))) {
  ...
  void render() {
    background(64);
    mapRenderer.draw(tileMap, mapOffsetX, mapOffsetY, tileWidth, tileHeight, settings.tileColors);
    drawExitDoor();
